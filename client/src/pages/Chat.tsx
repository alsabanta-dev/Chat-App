...
export default function Chat ({user, setUser}: Props){
  ...
  const handleContactSelect = async (contact: Object) => {
    try{
      const res = await axios('http://localhost:3000/api/v1/conversations', {
        method: 'POST',
        data: {
  ...
  const getConversations = async () => {
    ...
    try{
      const getConversationsRes = await axios.get(`http://localhost:3000/api/v1/conversations/user/${user._id}`)
